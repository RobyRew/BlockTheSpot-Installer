...
- The Pages feed `api/v1/windows-x64.json` now lists every build and adds `official` next to the mirror `url` when Spotify's link is known. The upstream LoadSpot table is still accepted as a fallback.
...
Verification: 55 core regression tests on both Linux and Windows, 12 catalog/site checks, and the native Windows startup/render test in both themes. The full install/reinstall process is not exercised on GitHub's hosted runners.
